        ...
        for train_idx, test_idx in outer_cv.split(X):
            ...
            for _ in range(n_repeats_cv1):
                ...
                for n_comp in n_components_range:
                    ...
                    mean_train_metrics = {k: np.mean([s[k] for s in train_metrics]) for k in train_metrics}
                    mean_val_metrics = {k: np.mean([s[k] for s in val_metrics]) for k in val_metrics}
...
def main(input_file, output_dir, repeat=50, run_permutation=False):
    ...
    for class1, class2, pair_name in class_pairs:
        ...
        dim_samples1 = samples1[samples1['dim'].between(1, 7)]
        dim_samples2 = samples2[samples2['dim'].between(1, 7)]
        print(f"{class1} samples in dim 1-7: {len(dim_samples1)}")
        print(f"{class2} samples in dim 1-7: {len(dim_samples2)}")
        ...
        for feature_type in feature_types:
            ...
            for i in range(repeat):
                # Determine minor and major group
                if len(dim_samples1) <= len(dim_samples2):
                    minor_group, major_group = dim_samples1, dim_samples2
                else:
                    minor_group, major_group = dim_samples2, dim_samples1
                # Get dim distribution in minor group
                minor_dim_counts = minor_group['dim'].value_counts().to_dict()
                # Sample from major group to match dim distribution
                sampled_major = []
                for dim_value, count in minor_dim_counts.items():
                    candidates = major_group[major_group['dim'] == dim_value]
                    if len(candidates) >= count:
                        sampled = candidates.sample(n=count, replace=False)
                    else:
                        sampled = candidates  # If not enough, take all
                    sampled_major.append(sampled)
                sampled_major = pd.concat(sampled_major)
                sampled_minor = minor_group
                # Combine
                balanced_data = pd.concat([sampled_major, sampled_minor]).reset_index(drop=True)
                balanced_data['disease'] = (balanced_data['group'] == class2).astype(int)
                if 'spc_de' in feature_type:
                    spectral_type = 'derivative'
                    base_feature = feature_type.replace('+spc_de','') if '+spc_de' in feature_type else 'spc'
                elif 'spc_rmR4' in feature_type:
                    spectral_type = 'rmR4'
                    base_feature = feature_type.replace('+spc_rmR4','') if '+spc_rmR4' in feature_type else 'spc'
                else:
